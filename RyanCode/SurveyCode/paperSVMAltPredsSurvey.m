...
% IF YOU WANT TO CHANGE THE AUDIO FEATURES BEING EXTRACTED SEE LINE 64.
...
saveLocation = '/Users/ryanbrummet/Documents/MATLAB/Audiology/SurveyStuff/Results/';

% Path to training and testing set .mat file.  Notice that you WILL STILL
% need to specify the correct response name below
processedDataLocation = 'Users/ryanbrummet/Documents/MATLAB/Audiology/AudioStuff/responseSets/ld2.mat';

% Survey predictors to include
surveyPredictorNames = {'patient','condition','ac','lc','tl','tf','vc','cp','rs','nl','nz'};

% SVM settings
degree = 1;
kernal = 2; %libsvm kernal setting
% Initial start values for grid search
startGammaValues = [.00001,.0001,.001,.01,.1,1,10,100,1000,10000,50000,1000000];
startCostValues = [.00001,.0001,.001,.01,.1,1,10,100,1000,10000,50000,1000000];
...
targetData = [dummyVars,table2array(dataTable(:,responseName))];
...
for outerFolds = 1 : outerCrossValFolds
    ...
    preOptG = NaN;
    iterationCount = 0;
    while optParam && iterationCount < maxIterationCount
...
save(strcat(saveLocation,responseName{1}),'cost','gamma','seed','cdfValues','targetData','degree','kernal','groupVars','startGammaValues','startCostValues','outerCrossValFolds');
