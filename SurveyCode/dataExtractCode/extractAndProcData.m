...
%  userSet (output vector): gives a vector of all user id's
    ...
    while(ischar(charArray))
        ...
        if size(startTime,2) == 8
            startTimeHour = (str2double(startTime(1,1))*10) + str2double(startTime(1,2));
            startTimeMin = (str2double(startTime(1,4))*10) + str2double(startTime(1,5));
            startTimeSec = (str2double(startTime(1,7))*10) + str2double(startTime(1,8));
        elseif size(startTime,2) == 7
            startTimeHour = str2double(startTime(1,1));
            startTimeMin = (str2double(startTime(1,3))*10) + str2double(startTime(1,4));
            startTimeSec = (str2double(startTime(1,6))*10) + str2double(startTime(1,7));
        else
            error('something went wrong with survey time removal; start'); 
        end
        if size(endTime,2) == 8
            endTimeHour = (str2double(endTime(1,1))*10) + str2double(endTime(1,2));
            endTimeMin = (str2double(endTime(1,4))*10) + str2double(endTime(1,5));
            endTimeSec = (str2double(endTime(1,7))*10) + str2double(endTime(1,8));
        elseif size(endTime,2) == 7
            endTimeHour = str2double(endTime(1,1));
            endTimeMin = (str2double(endTime(1,3))*10) + str2double(endTime(1,4));
            endTimeSec = (str2double(endTime(1,6))*10) + str2double(endTime(1,7));
        else
            disp('something went wrong with survey time removal; end'); 
        end
        ...
        if str2double(charArray(1,28)) == 1 && tempBool
            ...
            temp = zeros(1,9);
            for k = 1 : 9
                if str2double(charArray(1,tempIndexes(k))) ~= 50
