...
function [ RMSDNorm ] = findMapCoefGlobal(inputData, trainingSet, ...
    ...
    trainingSetTemp = trainingSet(find(trainingSet(:,1)),:);
    ...
    mapCoef = mean(mapCoefTemp,3);

    %map training/testing data using averged coef
    mappedTraining = trainingSet;
    mappedTesting = testingSet;
    for m = 1 : size(inputData(:,14:size(inputData,2)),2)
        for s = 1 : size(mappedTraining(:,1),1)
            convertedVal = 0;
            for g = deg: -1: 0
                convertedVal = convertedVal + (mappedTraining(s,13 + m)^g)*mapCoef(m,g + 1);
            end
            if convertedVal > 100
        ...
        for s = 1 : size(mappedTesting(:,1),1)
            convertedVal = 0;
            for g = deg: -1: 0
                convertedVal = convertedVal + (mappedTesting(s,13 + m)^g)*mapCoef(m,g + 1);
            end
            if convertedVal > 100
    ...
    for m = 1 : size(inputData(:,14:size(inputData,2)),2)
        RMSDNorm(2*m-1) = RMSDNormAttrTemp(m);
        RMSDNorm(2*m) = RMSDNormAttrPartition(m);
    end
