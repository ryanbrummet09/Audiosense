...
function [ dummyScores ] = dummyFunc( combinedScores, inputContexts )
    ...
    contextAmounts = [2 7 5 4 3 3 4 3 2 4 sum(~isnan(unique(combinedScores.condition)))];
    
    allContexts = {'hau', 'ac', 'lc', 'tf', 'vc', 'tl', 'nl', 'rs', 'cp', 'nz', 'condition'};
    ...
    dummyScores = zeros(sum(numColNeeded) + 1);
            ...
            if numColNeeded(1,j) ~= 0
                if ~isnan(combinedScores.(allContexts{j})(k))
                    if strcmp(allContexts{j},'condition')
                        dummyScores(k,sum(contextAmounts(1,1:j)) - contextAmounts(1,j) + find(combinedScores.(allContexts{j})(k) == conditions)) = 1;
                    else
                        dummyScores(k,sum(contextAmounts(1,1:j)) - contextAmounts(1,j) + combinedScores.(allContexts{j})(k)) = 1;
                    end
                end
            end
    ...
    %here we add demographic data in the order gender(dummy2{M,F}),
    ...
    %a+ is given a value of a for YearsSinceHLOnset and YearsOfHAUse.
    ...
    for k = 1 : size(numColNeeded,2) + 1
        if k <= size(numColNeeded,2)
            ...
            for j = 1 : numColNeeded(1,k)
                if k == 1
                    if j == 1
                        dummyScores.Properties.VariableNames{strcat('dummyScores',num2str(indexDemo))} = strcat('hau','false');
                    else
                        dummyScores.Properties.VariableNames{strcat('dummyScores',num2str(indexDemo))} = strcat('hau','true');
                    end
                elseif k == 2
