...
combineTech = 'SUMADJR';  %can be SUMADJ, AVG, MEDIAN, or STD
targetAttr = 'NoMap';  %NoMap for no map otherwise one of {'sp','le','ld2','lcl','ap','st'};
...
elseif strcmp(combineTech,'SUMADJR')
...
if removeFifties
    ...
    for k = 1 : size(attributes,2)
         unProcessedData(unProcessedData.(attributes{k})((unProcessedData.(attributes{k}) == 50) ...
...
elseif combineScoreTechnique == 2
    combinedAttr = round(nansum(normMappedAttr')' ./ sum((~isnan(normMappedAttr))')');
...
save(char(strcat('/Users/ryanbrummet/Documents/MATLAB/Audiology/compositeScores/',outputVarFileName)),'validationSet','trainingSet','normAVG','normSTD');
