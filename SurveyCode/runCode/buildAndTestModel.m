...
modelTech = 'interactions';  %options are constant, linear, interactions, purequadratic, quadratic, polyijk, or userBuilt
...
if strcmp(modelTech,'userBuilt')
    ...
    for k = 1 : size(depVars,2)
        if strcmp(model,'')
            model = strcat(strcat(strcat('patient','*'),'condition'),depVars{k});
        else
            model = strcat(strcat(model,{' + '}),strcat(strcat(strcat('patient','*'),'condition'),depVars{k}));
        end
    end
    model = char(model);
